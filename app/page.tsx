...
function HomeScreen({ onStatement, onLogout }: { onStatement: () => void; onLogout: () => void }) {
  ...
  return (
    <main className="bank-shell">
      ...
      <header className="topbar">
        ...
        <div className="top-actions"><IconButton label="Notifications"><Bell /></IconButton><IconButton label="Search"><Search /></IconButton><IconButton label="Power" onClick={() => setIsLogoutSheetOpen(true)}><Power /></IconButton></div>
      </header>
      ...
      <section className="card-summary" aria-label="Cards and offers"><div className="summary-card"><div><span>HDFC Bank</span><strong>Millennia Debit Card</strong><small>5260&nbsp; •••• &nbsp;••••&nbsp; 2444</small></div><div className="mastercard-mark"><i /><i /></div><div className="summary-card-footer"><span>Interest rate</span><b>0.83% p.m.</b><button>Manage Limit</button></div></div><div className="offer-banner"><div><p>Your HDFC Bank Credit Card is just a tap away.</p><strong>Check Offer.</strong></div><img src="/assets/sending-money.svg" alt="" /></div></section>
      <section className="payment-panel">
        <div className="drag-handle" />
        <div className="payment-heading"><h2>Payment Centre</h2><span>UPI: <b>******@okhdfcbank</b></span></div>
        <div className="send-card"><div><h3>Send Money</h3><p>via NEFT, IMPS, RTGS, UPI, &amp;<br />within HDFC Bank</p></div><button className="payee-button">+ New payee</button><input className="search-payee" type="text" inputMode="text" placeholder="Enter UPI ID / Mobile no. / Account no. / Name" aria-label="Enter UPI ID, mobile number, account number, or name" /></div>
        <div className="payment-actions payment-actions-expanded"><button onClick={() => setIsScannerOpen(true)} aria-haspopup="dialog"><QrCode /><span>Scan &amp; Pay</span></button><button><img src="/assets/send-amount-icon.svg" alt="" /><span>One-time transfer</span></button><button><img src="/assets/sending-money.svg" alt="" /><span>Self transfer</span></button><button><img src="/assets/send-amount-icon.svg" alt="" /><span>Send money abroad</span></button><button><img src="/assets/view-icon.svg" alt="" /><span>UPI collect request</span></button><button><img src="/assets/copy-icon.svg" alt="" /><span>Manage payee</span></button></div><section className="scheduled-transfers"><div><span>Scheduled Transfers</span><small>Manage your upcoming payments</small></div><button aria-label="Schedule new transfer"><strong>+</strong><span>Schedule new transfer</span></button></section>{isScannerOpen && <div className="scanner-backdrop" role="presentation" onClick={() => setIsScannerOpen(false)}><section className="scanner-modal" role="dialog" aria-modal="true" aria-labelledby="scanner-title" onClick={(event) => event.stopPropagation()}><header><div><p>Payment Centre</p><h2 id="scanner-title">Scan &amp; Pay</h2></div><button className="scanner-close" onClick={() => setIsScannerOpen(false)} aria-label="Close scanner"><X /></button></header><div className="scanner-frame"><div className="scanner-corner top-left" /><div className="scanner-corner top-right" /><div className="scanner-corner bottom-left" /><div className="scanner-corner bottom-right" /><QrCode /></div><p className="scanner-help">Align the QR code inside the frame to scan</p><button className="scanner-gallery">Choose from gallery</button></section></div>}</section>{isLogoutSheetOpen && <div className="logout-backdrop" role="presentation" onClick={() => setIsLogoutSheetOpen(false)}><section className="logout-sheet" role="dialog" aria-modal="true" aria-labelledby="logout-title" onClick={(event) => event.stopPropagation()}><div className="logout-sheet-handle" /><div className="logout-power-icon"><Power /></div><h2 id="logout-title">Are you sure you want to logout?</h2><div className="logout-actions"><button className="logout-cancel" onClick={() => setIsLogoutSheetOpen(false)}>Cancel</button><button className="logout-confirm" onClick={onLogout}>Log Out</button></div></section></div>}
      <nav className="dashboard-nav" aria-label="Primary navigation"><button className="active"><WalletCards /><span>Home</span></button><button><FileText /><span>Bills</span></button><button><Landmark /><span>Deposits</span></button><button><WalletCards /><span>Cards</span></button><button><UserRound /><span>Reach Us</span></button></nav>
    </main>
  )
}
...
function TransactionsScreen({ onBack }: { onBack: () => void }) {
  ...
  const [selectedMonth, setSelectedMonth] = useState(() => transactions[0]?.date.slice(0, 7) ?? '')
  ...
  return <main className="transactions-screen"><header className="transactions-header"><button className="back-button" onClick={onBack} aria-label="Back"><ArrowLeft /></button><h1>Transactions</h1></header><section className="statement-account"><div><span>Saving account</span><strong>{statementData.statement.maskedAccountNumber}</strong></div><div className="statement-balance"><span>Available balance</span><strong>{formatCurrency(latestTransaction.balance)}</strong></div></section><section className="transactions-drawer" aria-label="Monthly transaction statements"><div className="drawer-handle" /><div className="transaction-tools month-tools"><label htmlFor="statement-month">Statement month</label><select id="statement-month" value={selectedMonth} onChange={(event) => setSelectedMonth(event.target.value)} aria-label="Select statement month">{months.map((month) => <option key={month} value={month}>{new Intl.DateTimeFormat('en-US', { month: 'long', year: 'numeric' }).format(new Date(`${month}-01`))}</option>)}</select></div><div className="transaction-search"><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search transactions" aria-label="Search transactions" /></div><section className="transaction-list">{filteredTransactions.length ? filteredTransactions.map((tx, index) => <Transaction key={`${tx.date}-${tx.reference}-${index}`} {...tx} />) : <p className="empty-state">No transactions match these filters.</p>}</section></section></main>
}
...
function SplashScreen() {
  return <main className="splash-screen" aria-label="HDFC Bank"><div className="splash-logo" aria-hidden="true"><span>HDFC</span><small>BANK</small></div><h1>HDFC Bank</h1><div className="splash-loader" role="progressbar" aria-label="Loading" /></main>
}
...
function LoginScreen({ onLogin }: { onLogin: () => void }) {
  ...
  const getMpinClass = (index: number, digit: string) => `${index === focusedIndex ? 'focused ' : ''}${digit ? 'filled' : ''}`
